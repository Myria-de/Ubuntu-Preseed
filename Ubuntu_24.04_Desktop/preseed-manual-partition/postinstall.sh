...
# Allways use GDM3
if [ "$DESKTOPPKG" == "xubuntu-desktop^" ] || [ "$DESKTOPPKG" == "cinnamon-desktop-environment" ]
then
apt -y remove libva-wayland2
...
if [ "$FIREFOXDEB" == "yes" ] || [ "$CHROMIUMDEB" == "yes" ]
then
# Remove snapd
...
# Install snap-store later at first boot
# optional
echo '
[Unit]
...
[Service]
Type=oneshot
ExecStart=/usr/bin/snap install snap-store
ExecStartPost=/usr/bin/touch /root/snap-store-installed
RemainAfterExit=yes

[Install]
WantedBy=multi-user.target
' | tee /etc/systemd/system/firststart.service
